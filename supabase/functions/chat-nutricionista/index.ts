 ...
 *
 * @description
 * Edge Function conversacional para o nutricionista discutir plano alimentar com a IA.
 * Não possui tool calls - apenas conversa natural.
 *
 * @workflow
 * 1. Recebe: { aluno_id, mensagem_nutricionista }
 * 2. Busca conversation_id existente em instrucoes_nutricionista
 * 3. Se não existe: cria nova conversation + monta prompt com variáveis do aluno
 * 4. Se existe: continua conversation existente
 * 5. Envia mensagem do nutricionista para OpenAI
 * 6. Salva resposta da IA em instrucoes_ia
 * 7. Retorna resposta para o nutricionista
 *
 * @input
 * - aluno_id: UUID
...
serve(async (req)=>{
  ...
  console.log(`[CHAT-NUTRICIONISTA] 🚀 Início: ${startedAt}`);
  try {
    ...
    console.log(`[1] ✅ Mensagem (${mensagem_nutricionista.length} chars): "${mensagem_nutricionista.substring(0, 50)}..."`);
    ...
    if (!conversation_id) {
      ...
      const { data: promptData, error: promptErr } = await supabase.from("prompts_sistema").select("prompt_base").eq("chave", "prompt_nutricionista").maybeSingle();
      if (promptErr || !promptData) {
        throw new Error(`Prompt prompt_nutricionista não encontrado: ${promptErr?.message}`);
      }
      ...
      // 3.4: Buscar preferencias_alimentares
      const { data: prefAlimData, error: prefAlimErr } = await supabase.from("preferencias_alimentares").select("restricoes_alimentares, alimentos_nao_gosta, alimentos_favoritos, disposicao_cozinhar, orcamento_alimentar").eq("aluno_id", aluno_id).maybeSingle();
      console.log(`[3.4] ✅ Preferências alimentares carregadas`);
      // 3.5: Buscar preferencias_treino
      ...
      console.log(`[3.5] ✅ Preferências de treino carregadas`);
      // 3.6: Buscar goals
      ...
      console.log(`[3.6] ✅ Metas carregadas:`, goalsData);
      ...
      console.log(`[3.7.3] 📝 {{preferencias_alimentares}} (${prefAlimJson.length} chars): ${prefAlimJson.substring(0, 100)}...`);
      ...
      console.log(`[3.7.4] 📝 {{preferencias_treino}} (${prefTreinoJson.length} chars): ${prefTreinoJson.substring(0, 100)}...`);
      prompt_final = prompt_final.replace(/\{\{preferencias_treino\}\}/g, prefTreinoJson);
      const metasJson = JSON.stringify(goalsData || {}, null, 2);
      console.log(`[3.7.5] 📝 {{metas}} (${metasJson.length} chars): ${metasJson.substring(0, 100)}...`);
    ...
    // ============================================
    console.log(`[4] 📤 Enviando mensagem do nutricionista...`);
